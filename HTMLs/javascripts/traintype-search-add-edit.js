...
function showAllTrainType() {
  fetch("http://localhost:3000/agent/traintype/all/JSON")
    .then((res) => res.json())
...
function loadTrainTypeTable(data) {
  ...
  data.forEach(function ({
    ...
    tableHtml += `<td data-id=${train_type_id}>${train_type_description}</td>`;
...
function searchTrainType() {
  ...
  fetch("http://localhost:3000/agent/traintype/" + searchValue.toLowerCase())
    .then((res) => res.json())
    // .then((data) => console.log(data));
    .then((data) => {
      if (data.error) {
...
function deleteRowById(id) {
  fetch("http://localhost:3000/agent/traintype/delete/" + id, {
    method: "DELETE",
  })
    .then((res) => res.json())
    .then((data) => {
...
function handleEditRow(id) {
  ...
  document.getElementById("update-traintype-description-input").dataset.id = id;
  document.getElementById("update-traintype-description-input").value =
    tempArray[1];
...
updateBtn.onclick = function () {
  ...
  const newTrainTypeDescription = document.getElementById(
    "update-traintype-description-input"
  );

  // console.log(newtraintypeName.dataset.id);
  fetch("http://localhost:3000/agent/traintype/update", {
    method: "PATCH",
    headers: {
      "Content-type": "application/json",
    },
    body: JSON.stringify({
      ...
      newTrainTypeDescription: newTrainTypeDescription.value,
    }),
  })
    .then((res) => res.json())
    .then((data) => {
