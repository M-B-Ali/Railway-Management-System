...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("http://localhost:3000/agent/state/all/JSON")
    .then((res) => res.json())
    ...
    .then(() => {
      fetch("http://localhost:3000/agent/zone/all/JSON")
        .then((res) => res.json())
        // .then((data) => console.log(data));
        .then((data) => {
          if (data.error) {
...
function loadStatesList(data) {
  ...
  let statesListHtml = `<option value="0">Unknown</option>`;
  if (data.length === 0) {
...
function loadZonesList(data) {
  ...
  let zonesListHtml = `<option value="0">Unknown</option>`;
  if (data.length === 0) {
...
function showAllStation() {
  fetch("http://localhost:3000/agent/station/all/JSON")
    .then((res) => res.json())
    // .then((data) => console.log(data));
    .then((data) => {
      if (data.error) {
      ...
      console.log(data);
      loadStationTable(data["data"]);
    });
  //   loadStationTable([]);
  console.log("testing");
}

function loadStationTable(data) {
...
function searchStation() {
  ...
  fetch("http://localhost:3000/agent/station/" + searchValue.toLowerCase())
    .then((res) => res.json())
    // .then((data) => console.log(data));
    .then((data) => {
      if (data.error) {
...
function deleteRowById(id) {
  fetch("http://localhost:3000/agent/station/delete/" + id, {
    method: "DELETE",
  })
    .then((res) => res.json())
    .then((data) => {
...
updateBtn.onclick = function () {
  ...
  fetch("http://localhost:3000/agent/station/update", {
    method: "PATCH",
    headers: {
      "Content-type": "application/json",
    },
