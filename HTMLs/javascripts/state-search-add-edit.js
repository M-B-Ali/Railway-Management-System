...
function searchState() {
  ...
  fetch("http://localhost:3000/agent/state/" + searchValue.toLowerCase())
    .then((res) => res.json())
    // .then((data) => console.log(data));
    .then((data) => {
      if (data.error) {
...
function deleteRowById(id) {
  fetch("http://localhost:3000/agent/state/delete/" + id, {
    method: "DELETE",
  })
    .then((res) => res.json())
    .then((data) => {
...
updateBtn.onclick = function () {
  ...
  fetch("http://localhost:3000/agent/state/update", {
    method: "PATCH",
    headers: {
      "Content-type": "application/json",
    },
